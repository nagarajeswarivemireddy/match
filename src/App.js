...
class App extends Component {
  ...
  getScoreCard = () => {
    ...
    return <ResultCard score={score} homePage={this.homePage} />
  }

  topScore = () => {
    this.setState({matchInProgress: false})
  }

  randomImage = id => {
    const {clickedImagesList, score, time} = this.state

    const l = imagesList.length

    const randomIndex = Math.floor(l * Math.random())

    const isThere = clickedImagesList.includes(id)

    const len = clickedImagesList.length
  ...
  render() {
    ...
    const filteredImagesList = this.filterdImages()
      ...
      <div className="bg-container">
        ...
        <div>
          {matchInProgress ? (
            <div>
              <div className="card1">
                <img src={image} alt="match" className="mainImage" />
              </div>
